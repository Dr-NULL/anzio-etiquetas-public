export class Parser {
  ...
  public toInt(min?: number, max?: number) {
  ...
  public toFloat(dec?: number, min?: number, max?: number) {
    const rejex = this.rawValue.match(/(^-[0-9]+|[0-9]+)/gi);
    if (rejex == null) {
      return 0;
    }
  ...
  toString(dec?: number, min?: number, max?: number) {
    const match = this.toFloat(dec, min, max)
      .toString()
      .match(/(\+|-)?[0-9]+/gi);
